...
const {
  ...
  <main className="py-10 flex justify-center items-center min-h-screen bg-gray-100">
    <div className="w-full max-w-3xl p-8 bg-white rounded-lg shadow-lg transform transition duration-500 hover:scale-105">
      <h1 className="mb-6 text-3xl font-bold text-center text-gray-800">Editeaza Taxă Facultate</h1>

      <form onSubmit={handleSubmit(processForm)} className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="flex flex-col">
          <label className="text-gray-700 font-semibold  text-center">Valoare</label>
          <input
            {...register('value')}
            type="number"
          ...
          />
          {errors.value && (
            <p className="text-xs italic text-red-600">{errors.value.message}</p>
          )}
        </div>

        <div className="flex flex-col">
          <label className="text-gray-700 font-semibold  text-center">Ciclu de studiu</label>
          <Controller
            control={control}
            name="studyCycle"
            render={({ field: { onChange, onBlur, value, ref } }) => (
              <Select
                ...
                onBlur={onBlur}
                value={selectStudyCycles.find((option) => option.value === value)}
          ...
          />
          {errors.studyCycle && (
            <p className="text-xs italic text-red-600">{errors.studyCycle.message}</p>
          )}
        </div>

        <div className="flex flex-col">
          <label className="text-gray-700 font-semibold  text-center">Alege o facultate</label>
          <Controller
            control={control}
            name="facultyId"
            render={({ field: { onChange, onBlur, value, ref } }) => (
              <Select
                ...
                onChange={(newValue) => onChange(newValue?.value)}
                onBlur={onBlur}
                value={selectFaculties.find((option) => option.value === value)}
                className="basic-single w-full"
                classNamePrefix="select"
                isDisabled={isDisabled}
                isLoading={isLoading}
          ...
          />
          {errors.facultyId && (
            <p className="text-xs italic text-red-600">{errors.facultyId.message}</p>
          )}
        </div>

        <div className="flex flex-col">
          <label className="text-gray-700 font-semibold  text-center">Tip taxa facultate</label>
          <Controller
            control={control}
            name="facultyTaxType"
            render={({ field: { onChange, onBlur, value, ref } }) => (
              <Select
                ...
                onChange={(newValue) => onChange(newValue?.value)}
                onBlur={onBlur}
                value={selectFacultyTaxTypes.find((option) => option.value === value)}
                className="basic-single w-full"
                classNamePrefix="select"
                isDisabled={isDisabled}
                isLoading={isLoading}
          ...
          {errors.facultyTaxType && (
            <p className="text-xs italic text-red-600">{errors.facultyTaxType.message}</p>
          )}
        </div>

        <div className="flex flex-col col-span-2">
          <label className="text-gray-700 font-semibold  text-center">Observatii (RO)</label>
          <textarea
            {...register('remarksRo')}
        ...
        <div className="flex flex-col col-span-2">
          <label className="text-gray-700 font-semibold  text-center">Observatii (EN)</label>
          <textarea
            {...register('remarksEn')}
            className="mt-1 block w-full border border-gray-300 rounded-md p-3 shadow-sm focus:border-blue-500 focus:ring-blue-500"
