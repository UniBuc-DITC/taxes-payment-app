...
const FormButton: FC<FormButtonProps> = ({ text, icon, onClick }) => {
  return (
    <button
      onClick={onClick}
      type="submit"
      className="flex items-center justify-center px-4 py-2 text-white bg-green-500 rounded hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50"
    >
      {icon} {text}{" "}
      {/* Directly render the icon without checking, as React handles null/undefined gracefully */}
    </button>
  );
};
