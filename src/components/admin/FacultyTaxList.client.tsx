...
export default function FacultyTaxList({ taxFaculties, faculties }: Props) {
  const tuitionValues = taxFaculties.filter(
    (value) => value.facultyTaxType === "tuition",
  );
  const admissionValues = taxFaculties.filter(
    (value) => value.facultyTaxType === "admission",
  );
          ...
          <div>
            ...
            {tuitionValues.map((value) => (
              ...
              >
                ...
                <div>
                  <strong>Ciclu de Studiu: </strong>{" "}
                  {getLabel(value.studyCycle)}
                </div>
                <div>
                  <strong>Facultate :</strong>{" "}
                  {
                    faculties.find((faculty) => faculty.id === value.facultyId)
                      ?.nameRo
                  }
                </div>
                <div>
                  <strong>Observații (RO) :</strong> {value.remarksRo}
                </div>
                <div>
                  <strong>Observații (EN) :</strong> {value.remarksEn}
                </div>
