          ...
          <div className="bg-white shadow overflow-hidden sm:rounded-lg">
            <ul className="divide-y divide-gray-200">
              {accounts.map((account) => (
                <li key={account.id} className="px-6 py-4 flex justify-between items-center hover:bg-gray-50">
                  <div className="flex-1">
                    <p className="text-sm font-medium text-gray-900">{account.name}</p>
                    <p className="text-sm text-gray-500">{account.description || 'No description'}</p>
                  </div>
                  <div className="flex items-center">
                    <Link href={{ pathname: "euplatesc-accounts/update", query: { 
                        id: account.id, 
                        name: account.name,
                        description: account.description,
                        merchantId: account.merchantId,
                        secretKey: account.secretKey,
                        didacticPremiumCardOnly: account.didacticPremiumCardOnly, } }} passHref>
