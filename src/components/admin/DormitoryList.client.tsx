      ...
      <main className="flex min-h-screen bg-gray-100 p-10 flex-col items-center justify-start">
        <div className="w-full max-w-4xl">
          <div className="mb-4 flex justify-between items-center">
            <h1 className="text-2xl font-bold text-gray-800">Gestionare camine</h1>
          ...
          <div className="bg-white shadow overflow-hidden sm:rounded-lg">
            <ul className="divide-y divide-gray-200">
              {dormitories.map((dormitory) => (
                <li key={dormitory.id} className="px-6 py-4 flex justify-between items-center hover:bg-gray-50">
                  <span className="flex-1 text-sm font-medium text-gray-900">{dormitory.name}</span>
                  <div className="flex items-center">
                    <Link href={{ pathname: "dormitories/update", query: { id: dormitory.id, name: dormitory.name, accountId: dormitory.euplatescAccountId } }} passHref>
